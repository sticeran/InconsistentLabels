...
# time2 = 'Mon Nov 2 13:46:02 2015'
# ttt = datetime.datetime.strptime(time2,'%a %b %d %H:%M:%S %Y')
# print()

# str1 = "123/*adfadf adf ad456";#可以
# str2 = "123/*adfadf adf ad*/fff*/456";#fff*/不可以
# str3 = "123 /*fff /*fff /*adfadf adf ad*/ fff*/ fff*/ 123 /*adf*/ 456 /*fff /*fff /*adfadf adf ad*/ fff*/ fff*/ //qwer";
# str4 = "123/*adfadf adf ad 456 //qwer";
 
# aaa = re.sub("/[*](.*?)[*]/", '', str1, re.S);#首先得去掉一行中的/**/包裹起来的注释
# aaa = "@www 123 @456 789 @qqq\n"
# bbb = re.sub("@(\S)*|@(\S)*$", '', aaa, re.S);#然后得去掉一行中的//包裹起来的注释
#对于str3，用进栈出栈的思想消灭掉。但是麻烦啊。假设/**/不存在嵌套，在实际中也很少出现嵌套，那么只用aaa和bbb的过滤就够了。
# print()
...
def dividedIntoPairOfModifiedLines(rep,i_position_start,i_position_end):
    ...
    j_underAtAt = 0;#记录@@下第几行，minus_start_line + j_underAtAt = 在原文件中的正确行号
...
#在这一步得到时间，空文件不写。
# 获取有效的被修复的java文件名，有效的被修改的行的行号
if __name__ == "__main__":
    #project_list = ['Hadoop Common','Camel','OpenJPA','Mahout','Tuscany',
    #"avro","cxf","wicket","groovy","pdfbox","bigtop","openmeetings","Apache NiFi","flink","flume","sqoop"]
    projectName_list = ["zeppelin","shiro","maven","flume","mahout"];
#     projectName_list = ["shiro"];#调试用
    
    for project in projectName_list:
        fileNames_commits = os.listdir(r'D:\workspace\mixed-workspace\mySZZ\git_show_bugFixingCommitsID\%s'%project)
    #     fileNames_commits = '123';#调试用
        for str_i_filename in fileNames_commits:
#             str_i_filename = "zeppelin_eb5c275ed3a06429802869991e5cda835a478503.txt";#调试用
            try:
                txt = codecs.open(r'D:\workspace\mixed-workspace\mySZZ\git_show_bugFixingCommitsID\%s\%s'%(project,str_i_filename),'rb','utf_16_le')#UTF-16LE，其后缀是 LE 即 little-endian，小端的意思。小端就是将高位的字节放在高地址表示。
    #             txt = codecs.open(r'D:\workspace\mixed-workspace\mySZZ\git_show_bugFixingCommitsID\%s\%s'%(project,str_i_filename),'r')
                ...
                start = str_i_filename.index('_')+1
                end = str_i_filename.index('.')
    #             print (start, end)
                commitsha = str_i_filename[start:end]
    #             print (commitsha)
    ...
    #                 print (rep[i])#打印当前语句
                    ...
                    if bool_existModifiedJavaFile:
                        if rep[i][0:2] == '@@':
                            ...
                            if rep[i-2].find("--- /dev/null")==-1:#没找到
...
#                             wtxt.write('\n')
                ...
                txt.close()
                if wtxt_contents != '':
                    wtxt = open(r'D:\workspace\mixed-workspace\mySZZ\buggyFileAndLineNum_from_git_show\%s\buggyFileAndLineNum_%s'%(project,str_i_filename),'w')
