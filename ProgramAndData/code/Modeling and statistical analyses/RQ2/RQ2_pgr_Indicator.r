...
# list_list_datasetPath = list(c("JIRA-HA-2019"))
...
list_read_fileName_random = c("(random)datapoints_filtered.csv")
      ...
      for (i in 1:length(list_combinedPatterns))
      {
        ...
        for (i_targetIndicator in columns_targetIndicator)
        {
          ...
          for (i_dataset_style in 1:length(list_dataset_style))
          {
            ...
            i_read_fileName_random = list_read_fileName_random[i]
            ...
            file_random = paste(path_common,i_combinedPatterns,i_filteringMethod,i_read_fileName_random,sep = '/')# 随机模型路径
            # 数据集
            ...
            df_file_random = read.table(file=file_random, header=TRUE, sep=",")
            ...
            vector_random = df_file_random[,i_targetIndicator]
            
            # data <- abs((vector_comparison-vector_benchmark)/vector_benchmark)*100
            data <- abs(((vector_comparison-vector_random)-(vector_benchmark-vector_random))/(vector_benchmark-vector_random))*100
